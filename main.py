...
class AntelopeNetClient:
    def __init__(self, peer, chain_id, node_id):
        self.peer = peer
    ...
    def receive_messages(self):
        try:
            while True:
                ...
                threading.Thread(
                    target=(
                      lambda: self.handle_net_message(data)
                    ), daemon=True
                ).start()
        finally:
    ...
    def handle_net_message(self, data):
        ...
        print(
          f'Received message type {message_type} with {message_length} bytes'
        )
    ...
    def perform_handshake(self):
        ...
        print("Final handshake message:", message, "\n", message.hex())
        try:
    ...
    def send_time_message(self):
        ...
        print(header.offset, body.offset)
        message = header.filled + body.filled
        print("Time:", message.hex())
        self.socket.send(message)
